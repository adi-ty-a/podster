...
import { Startpod } from "./home/startpod"
import { WatchDemo } from "./home/watchDemo"
import { Socials } from "./home/socials"
import MenuBox from "./home/menubox";
import { X } from "lucide-react";
import { useState } from "react";
import { useRouter } from "next/navigation";
import { StartFreeTrail } from "./home/startfreetrial";
import FeatureSection from "./testupload/page";
import { Chatbox, ClearnUi } from "./home/features";

const Home = () => {
  const [show,setclose] = useState(false);
  const router = useRouter()
    return (
      <div className=" relative w-full h-fit flex justify-center bg-[#FAFDFF] overflow-x-hidden">
        {show && <div className="absolute inset-0 w-full h-full bg-white/80 backdrop-blur-lg z-30 md:hidden ">
          ...
          <div className=" bg-[#FAFDFF] h-full max-w-[1200px] w-full pt-1 px-1 pb-0 flex flex-col items-center">
            {/* headernav */}
       ...
       <div className="w-full relative pointer-events-none">
          ...
          />
            ...
            {/* features */}
<div className="h-fit py-[60px] md:py-[200px] w-screen overflow-x-hidden bg-black flex flex-col items-center justify-center gap-[40px] md:gap-[60px]">
  <div className="flex flex-col items-center justify-center gap-3 size-fit pb-[60px] md:pb-[120px] px-6 text-center">
    <div className="text-[28px] leading-[30px] md:text-[38px] md:leading-[38px] font-bold text-center bg-[linear-gradient(180deg,_#fff_-12%,_#9F9F9F_134%)] bg-clip-text text-transparent">
      Build for Conversations
    </div>
    <div className="text-[18px] leading-[20px] md:text-[24px] md:leading-[24px] font-bold text-center bg-[linear-gradient(180deg,_#fff_-12%,_#9F9F9F_134%)] bg-clip-text text-transparent">
      That Matter
    </div>
  </div>

  <div className="w-screen h-full flex md:[perspective:1000px] flex-col items-center justify-center md:bg-[repeating-linear-gradient(90deg,rgba(255,255,255,0.2)_0_6px,transparent_6px_12px),repeating-linear-gradient(90deg,rgba(255,255,255,0.2)_0_6px,transparent_6px_12px)] md:bg-[length:100%_1px,100%_1px] md:bg-[position:top,bottom] md:bg-no-repeat gap-[64 px]">

      {/* Row 1 */}
      <div className="flex flex-col md:flex-row h-full w-full md:bg-[repeating-linear-gradient(90deg,rgba(255,255,255,0.2)_0_6px,transparent_6px_12px)] md:bg-[length:100%_1px] md:bg-[position:bottom] md:bg-no-repeat gap-[32px]">
        <div className="flex-[1.1] py-[20px] md:py-[80px] h-full flex justify-center md:justify-end pr-0 md:pr-[90px]">
          <ClearnUi/>
        </div>
        <div className="hidden md:block w-[1px] bg-[repeating-linear-gradient(180deg,rgba(255,255,255,0.2)_0_6px,transparent_6px_12px)] bg-[length:1px_100%] bg-right bg-no-repeat"></div>
        <div className="flex-1 flex flex-col justify-center px-6 md:px-0 md:pl-[80px] gap-[16px] md:gap-[20px] pb-[30px] md:pb-0 md:-translate-y-10">
      ...
      {/* Row 2 */}
      <div className="flex flex-col md:flex-row w-full h-full gap-[32px]">
        <div className="flex-1 flex flex-col justify-center px-6 md:px-0 md:pr-[80px] gap-[16px] md:gap-[20px] pb-[30px] md:pb-0 md:-translate-y-10 items-start md:items-end order-2 md:order-1">
            <div className="text-[#ACACAC] w-full md:w-[60%]">
              <div className="translate-x-12 md:-translate-x-4  -translate-y-1 size-[15px] bg-[#404040]"/>
              <div className="text-[12px] md:text-[16px] px-[64px]">
        ...
        <div className="hidden md:block w-[1px] md:bg-[repeating-linear-gradient(180deg,rgba(255,255,255,0.2)_0_6px,transparent_6px_12px)] bg-[length:1px_100%] bg-right bg-no-repeat"></div>
        <div className="flex-1 py-[20px] md:py-[80px] pl-0 md:pl-[80px] flex justify-center md:justify-start order-1 md:order-2">
          <Chatbox/>
        </div>
      </div>
  </div>
