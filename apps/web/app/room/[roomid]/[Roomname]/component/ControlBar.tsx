...
import { Recording } from "@/app/webrtc/recording"
import { useChat, useRecording} from "@/app/store"
...
export const ControlBar=({data}:ControlBarData)=>{
  //zustand recording state 
    ...
    useEffect(()=>{
        console.log("manager changed", manager);
      if (!manager) return
      manager.recording.setupEndRecordingListener(onVideoStop);
      return ()=>{
        manager.recording.removeEndRecordingListener(onVideoStop);
      }
    },[manager]);
